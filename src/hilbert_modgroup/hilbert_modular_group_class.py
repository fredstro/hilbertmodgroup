...
class HilbertModularGroup_class(LinearMatrixGroup_generic):
    ...
    def __init__(self, base_ring, sage_name, latex_string):
        ...
        """
        if base_ring != ZZ and (
            not isinstance(base_ring, Order) or not base_ring.number_field().is_totally_real()
        ):
            raise ValueError(
                "Input (={0}) can not be used to create a Hilbert modular group. "
    ...
    def to_json(self):
        ...
        EXAMPLES::
            ...
            sage: from hilbert_modgroup.all import HilbertModularGroup
            sage: H = HilbertModularGroup(5)
            sage: H.to_json() # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
            ...
            {'_cls': 'hilbert_modgroup.hilbert_modular_group_class.HilbertModularGroup_class_with_category',
             'base_ring': '{"__type__": "number_field_order", "number_field": {"polynomial": "x^2 - 5", "name": "Number Field in a with defining polynomial x^2 - 5 with a = 2.236067977499790?", "names": ["a"], "__type__": "number_field", "embedding": "2.23606797749979"}, "is_maximal": true}',
             'latex_string': 'PSL(2, \\text{\\texttt{Maximal{ }Order{ }generated{ }by{ }1/2*a{ }+{ }1/2{ }in{ }Number{ }Field{ }in{ }a{ }with{ }defining{ }polynomial{ }x{\\char`\\^}2{ }{-}{ }5{ }with{ }a{ }={ }2.236067977499790?}})',
             'sage_name': 'Hilbert Modular Group PSL(2) over Maximal Order generated by 1/2*a + 1/2 in Number Field in a with defining polynomial x^2 - 5 with a = 2.236067977499790?'}
        """
        from comp_manager.utils.serialization import serialize

        full_class_name = f"hilbert_modgroup.hilbert_modular_group_class.{self.__class__.__name__}"
        return {
            "_cls": full_class_name,
            "base_ring": serialize(self.base_ring()).decode("utf-8"),
    ...
    def from_json(cls, data):
        ...
        EXAMPLES::
            ...
            sage: from comp_manager.utils.serialization import deserialize, serialize
            sage: H = HilbertModularGroup(5)
            sage: H.to_json() # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
            {'_cls': 'hilbert_modgroup.hilbert_modular_group_class.HilbertModularGroup_class_with_category',
            'base_ring': '{"__type__": "number_field_order", "number_field": {"polynomial": "x^2 - 5", "name": "Number Field in a with defining polynomial x^2 - 5 with a = 2.236067977499790?", "names": ["a"], "__type__": "number_field", "embedding": "2.23606797749979"}, "is_maximal": true}',
            'latex_string': 'PSL(2, \\text{\\texttt{Maximal{ }Order{ }generated{ }by{ }1/2*a{ }+{ }1/2{ }in{ }Number{ }Field{ }in{ }a{ }with{ }defining{ }polynomial{ }x{\\char`\\^}2{ }{-}{ }5{ }with{ }a{ }={ }2.236067977499790?}})',
            'sage_name': 'Hilbert Modular Group PSL(2) over Maximal Order generated by 1/2*a + 1/2 in Number Field in a with defining polynomial x^2 - 5 with a = 2.236067977499790?'}
            sage: HilbertModularGroup_class.from_json(H.to_json()) # doctest: +NORMALIZE_WHITESPACE
                Hilbert Modular Group PSL(2) over Maximal Order generated by 1/2*a + 1/2 in Number
                Field in a with defining polynomial x^2 - 5 with a = 2.236067977499790?
            sage: serialize(H) == json.dumps(H.to_json()).encode('utf-8')
            True

        """
        from comp_manager.utils.serialization import deserialize

        if isinstance(data, str):
            data = json.loads(data)
        if not isinstance(data, dict):
            raise ValueError("Input `data` must be a dict or JSON representation of a dict.")
        base_ring = deserialize(data["base_ring"].encode("utf-8"))
        sage_name = data["sage_name"]
        latex_string = data["latex_string"]
        return cls(base_ring, sage_name, latex_string)

    def _cache_key(self):
        """
        Return a tuple that can be used as a cache key.

        EXAMPLES::

            sage: from hilbert_modgroup.all import HilbertModularGroup
            sage: H = HilbertModularGroup(5)
            sage: H._cache_key() # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
            (Maximal Order generated by 1/2*a + 1/2 ...)
        """
        return (self.base_ring(), self._name_string, self._latex_string)
    ...
    def __eq__(self, other):
        ...
        EXAMPLES::
            ...
            sage: from hilbert_modgroup.all import HilbertModularGroup
            sage: H1 = HilbertModularGroup(5)
            sage: H2 = HilbertModularGroup(5)
            sage: H1 == H2
            True
            sage: H3 = HilbertModularGroup(7)
            sage: H1 == H3
            False

        """
        if not isinstance(other, HilbertModularGroup_class):
            return False
        return all(
            [
                self.base_ring().number_field().polynomial()
                == other.base_ring().number_field().polynomial(),
                str(self.base_ring().gens()) == str(other.base_ring().gens()),
    ...
    def generators(self, algorithm="standard"):
        ...
        if algorithm == "standard":
            ...
            raise ValueError(
                f"Unknown algorithm '{algorithm}'. Expected one of 'standard' or 'elementary'"
            )
        return gens
