...
def plot_polygon(p: Polyhedron, basis: tuple, action: str = 'show', filename: str = '',
    ...
    sage: plot_polygon(p,K1.fractional_ideal(1).integral_basis(),action='show',**norm_args)
...
plot_polygon(p,[1,1,1],action='show',ticks=[2,2,2],polygon=False,inside_polygon=True,xmin=-2,xmax=2,
    ...
    if n == 2:
        add_pts_degree2(G, basis, xmin, xmax, ymin, ymax, inside_polygon, p)
        G.set_axes_range(xmin - 1, xmax + 1, ymin - 1, ymax + 1)
        G.axes_labels(labels)

    else:
        add_pts_degree3(G, basis, xmin, xmax, ymin, ymax, zmin, zmax, inside_polygon, curve_map, p,
                        norm_bound)
    ...
    if action == 'show':
        return G.matplotlib(**show_options)
...
def add_pts_degree2(G: Graphics, basis: tuple, xmin: float, xmax: float, ymin: float, ymax: float,
                    inside_polygon: bool, p: Polyhedron) -> None:
    """
...
def add_pts_degree3(G, basis: tuple, xmin: float, xmax: float, ymin: float, ymax: float,
                    ...
                    curve_map: tuple = None, p: Polyhedron = None, norm_bound: float = 1.0):
    """
    ...
    EXAMPLES:
            ...
            sage: inside_polygon = False
            sage: curve_map = None
            sage: add_pts_degree3(G, basis, xmin, xmax, ymin, ymax, zmin, zmax)
